...
SocketAddress::SocketAddress(void *platform_data) : _platform_data(platform_data) {}
...
u_result SocketAddress::setAddressFromString(const char *address_string, SocketAddress::address_type_t type) {
    ...
    switch (type) {
        case ADDRESS_TYPE_INET:
            reinterpret_cast<sockaddr_storage *>(_platform_data)->ss_family = AF_INET;
            ans = inet_pton(AF_INET, address_string, &reinterpret_cast<sockaddr_in *>(_platform_data)->sin_addr);
            break;

        case ADDRESS_TYPE_INET6:

            reinterpret_cast<sockaddr_storage *>(_platform_data)->ss_family = AF_INET6;
            ans = inet_pton(AF_INET6, address_string, &reinterpret_cast<sockaddr_in6 *>(_platform_data)->sin6_addr);
            break;
...
u_result SocketAddress::getAddressAsString(char *buffer, size_t buffersize) const {
    ...
    switch (net_family) {
        case AF_INET:
            ans = inet_ntop(net_family, &reinterpret_cast<const sockaddr_in *>(_platform_data)->sin_addr, buffer,
                            buffersize);
            break;

        case AF_INET6:
            ans = inet_ntop(net_family, &reinterpret_cast<const sockaddr_in6 *>(_platform_data)->sin6_addr, buffer,
                            buffersize);
...
class _single_thread StreamSocketImpl : public StreamSocket {
   public:
    ...
    virtual u_result enableKeepAlive(bool enable) {
        ...
        return ::setsockopt(_socket_fd, SOL_SOCKET, SO_KEEPALIVE, &bool_true, sizeof(bool_true)) ? RESULT_OPERATION_FAIL
                                                                                                 : RESULT_OK;
    }

    virtual u_result enableNoDelay(bool enable) {
        int bool_true = enable ? 1 : 0;
        return ::setsockopt(_socket_fd, IPPROTO_TCP, TCP_NODELAY, &bool_true, sizeof(bool_true)) ? RESULT_OPERATION_FAIL
                                                                                                 : RESULT_OK;
    }
