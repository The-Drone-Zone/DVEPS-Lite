...
SocketAddress::SocketAddress(void *platform_data) : _platform_data(platform_data) { _checkWSAStartup(); }
...
SocketAddress::~SocketAddress() { delete reinterpret_cast<sockaddr_storage *>(_platform_data); }
...
u_result SocketAddress::setAddressFromString(const char *address_string, SocketAddress::address_type_t type) {
    ...
    switch (type) {
        case ADDRESS_TYPE_INET:
            reinterpret_cast<sockaddr_storage *>(_platform_data)->ss_family = AF_INET;
            ans = _inet_pton(AF_INET, address_string, &reinterpret_cast<sockaddr_in *>(_platform_data)->sin_addr);
            break;

        case ADDRESS_TYPE_INET6:

            reinterpret_cast<sockaddr_storage *>(_platform_data)->ss_family = AF_INET6;
            ans = _inet_pton(AF_INET6, address_string, &reinterpret_cast<sockaddr_in6 *>(_platform_data)->sin6_addr);
            break;
...
u_result SocketAddress::getAddressAsString(char *buffer, size_t buffersize) const {
    ...
    switch (net_family) {
        case AF_INET:
            ans = _inet_ntop(net_family, &reinterpret_cast<const sockaddr_in *>(_platform_data)->sin_addr, buffer,
                             (int)buffersize);
            break;

        case AF_INET6:
            ans = _inet_ntop(net_family, &reinterpret_cast<const sockaddr_in6 *>(_platform_data)->sin6_addr, buffer,
                             (int)buffersize);
...
class _single_thread StreamSocketImpl : public StreamSocket {
   public:
    StreamSocketImpl(SOCKET fd) : _socket_fd(fd) {
        assert(fd >= 0);
        int bool_true = 1;
    ...
    virtual ~StreamSocketImpl() { closesocket(_socket_fd); }
    ...
    virtual u_result getLocalAddress(SocketAddress &localaddr) {
        struct sockaddr *addr = reinterpret_cast<struct sockaddr *>(
            const_cast<void *>(localaddr.getPlatformData()));  // donnot do this at home...
        assert(addr);
    ...
    virtual StreamSocket *accept(SocketAddress *pairAddress) {
        ...
        SOCKET pair_socket = ::accept(
            _socket_fd,
            pairAddress ? reinterpret_cast<struct sockaddr *>(const_cast<void *>(pairAddress->getPlatformData()))
                        : NULL,
            &addrsize);
    ...
    virtual u_result waitforIncomingConnection(_u32 timeout) { return waitforData(timeout); }
    ...
    virtual u_result getPeerAddress(SocketAddress &peerAddr) {
        struct sockaddr *addr = reinterpret_cast<struct sockaddr *>(
            const_cast<void *>(peerAddr.getPlatformData()));  // donnot do this at home...
        assert(addr);
    ...
    virtual u_result enableKeepAlive(bool enable) {
        ...
        return ::setsockopt(_socket_fd, SOL_SOCKET, SO_KEEPALIVE, (const char *)&bool_true, (int)sizeof(bool_true))
    ...
    virtual u_result enableNoDelay(bool enable) {
        int bool_true = enable ? 1 : 0;
        return ::setsockopt(_socket_fd, IPPROTO_TCP, TCP_NODELAY, (const char *)&bool_true, (int)sizeof(bool_true))
                   ? RESULT_OPERATION_FAIL
                   : RESULT_OK;
    }
...
class _single_thread DGramSocketImpl : public DGramSocket {
   public:
    DGramSocketImpl(SOCKET fd) : _socket_fd(fd) {
        assert(fd >= 0);
        int bool_true = 1;
    ...
    virtual ~DGramSocketImpl() { closesocket(_socket_fd); }
    ...
    virtual u_result setPairAddress(const SocketAddress *pairAddress) {
        ...
        const struct sockaddr *addr = pairAddress
                                          ? reinterpret_cast<const struct sockaddr *>(pairAddress->getPlatformData())
                                          : reinterpret_cast<const struct sockaddr *>(&unspecAddr);
        int ans = ::connect(_socket_fd, addr, (int)sizeof(sockaddr_storage));
        return ans ? RESULT_OPERATION_FAIL : RESULT_OK;
    }
    ...
    virtual u_result sendTo(const SocketAddress *target, const void *buffer, size_t len) {
        const struct sockaddr *addr =
            target ? reinterpret_cast<const struct sockaddr *>(target->getPlatformData()) : NULL;
    ...
    virtual u_result recvFrom(void *buf, size_t len, size_t &recv_len, SocketAddress *sourceAddr) {
        struct sockaddr *addr =
            (sourceAddr ? reinterpret_cast<struct sockaddr *>(const_cast<void *>(sourceAddr->getPlatformData()))
                        : NULL);
