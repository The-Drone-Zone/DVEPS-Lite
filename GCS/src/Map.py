...
class Map:
    ...
    def add_map(self):
        file_path = self.window_wrapper.display_input_box(
            "Enter the path to the map image you want to add:", title="Add Map"
        )
    ...
    def upload_map_image(self, path):
        ...
        self.window_wrapper.display_message(
    ...
    def open_map(self):
        if self.window_id in self.window_wrapper.windows:
            ...
            if self.first_open:
                window.geometry(
                    f"{self.img_width}x{total_height + 3*self.CONST_BUTTON_HEIGHT}"
                )
    ...
    def create_map(self):
        ...
        self.canvas = tk.Canvas(
            window, width=self.img_width, height=self.img_height, highlightthickness=0
        )
        self.canvas.pack(fill=tk.BOTH, expand=True)
        self.canvas.create_image(0, 0, image=self.photo, anchor=tk.NW)
        self.canvas.bind("<Button-1>", self.on_map_click)

        hide_window_btn = self.globals.Button_wrapper.create_button(
            window, 20, text="Hide Map", command=self.hide_map
        )
        hide_window_btn.pack(side=tk.BOTTOM, fill=tk.X)

        self.clear_marks_btn = self.globals.Button_wrapper.create_button(
            window, 20, text="Clear Marks", command=self.clear_marks
        )
        self.clear_marks_btn.pack_forget()

        self.make_route_btn = self.globals.Button_wrapper.create_button(
            window, 20, text="Make Route", command=self.make_route
        )
        self.make_route_btn.pack_forget()
